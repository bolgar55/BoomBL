...
- **Открыто для таска 07:** смонтировать `createGameOverScreen(...).show(score)` в момент `board.canFitAnywhere()===false`; вызывать `playClick/playLineClear/playGameOver` в игровом цикле; вызвать `sound.mountToggleButton()`; добавить CSS для `.gameover-overlay`/`.sound-toggle`
