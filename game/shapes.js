...
// «Неровные» фигуры — уголки-тримино (corner-1..4), зигзаги (S/Z-тетромино)
// и теперь ещё L- и Т-тетромино — тайлятся хуже прямых/прямоугольных фигур
// того же размера: по своей форме они заметно чаще оставляют после себя дыры
// в 1-2 клетки, которые потом некуда закрыть, и мешают когда-либо собрать
// полную очистку поля (игрок явно попросил снизить шанс и для L/Т тоже, не
// только уголков/зигзагов — «типа рандомно падало, но по типам фигур сделать
// шанс дропа»). Дело не в размере фигуры, а именно в её неровной форме —
// поэтому это не отдельная система «классов по размеру», а расширение уже
// работающего множителя веса. Не убираем совсем — только заметно снижаем вес
// выбора; фильтр «есть хоть одна допустимая позиция» их всё равно не касается.
...
/** Множитель веса для «неровных» фигур (уголки, зигзаги, L, Т) — 1 для всех остальных. */
